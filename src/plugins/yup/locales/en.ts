...
export const yupEn = {
    mixed: {
        ...
        notOneOf: ((ctx: MessageContext) => {
            const fieldName = ctx.linked(`yupFields.${ctx.named('path')}`)
            return capitalize(`${fieldName} is a value that is not: ${ctx.named('values')}`)
        }) as MessageFunctionCallable,
    ...
    string: {
        length: ((ctx: MessageContext) => {
            const fieldName = ctx.linked(`yupFields.${ctx.named('path')}`)
            return capitalize(`${fieldName} must have exact ${ctx.named('length')} characters`)
        }) as MessageFunctionCallable,
        ...
        max: ((ctx: MessageContext) => {
            const fieldName = ctx.linked(`yupFields.${ctx.named('path')}`)
            return capitalize(`${fieldName} must not exceed ${ctx.named('length')} characters`)
        }) as MessageFunctionCallable,
    ...
    number: {
        min: ((ctx: MessageContext) => {
            const fieldName = ctx.linked(`yupFields.${ctx.named('path')}`)
            return capitalize(`The ${fieldName} field must be at least ${ctx.named('min')}`)
        }) as MessageFunctionCallable,
        max: ((ctx: MessageContext) => {
            const fieldName = ctx.linked(`yupFields.${ctx.named('path')}`)
            return capitalize(`The ${fieldName} field can not be over ${ctx.named('max')}`)
        }) as MessageFunctionCallable,
    ...
    array: {
        min: ((ctx: MessageContext) => {
            const fieldName = ctx.linked(`yupFields.${ctx.named('path')}`)
            return capitalize(`The ${fieldName} field must have at least ${ctx.named('min')} items`)
        }) as MessageFunctionCallable,
        max: ((ctx: MessageContext) => {
            const fieldName = ctx.linked(`yupFields.${ctx.named('path')}`)
            return capitalize(
                `The ${fieldName} field must have less than or equal to ${ctx.named('max')} items`,
            )
        }) as MessageFunctionCallable,
        length: ((ctx: MessageContext) => {
            const fieldName = ctx.linked(`yupFields.${ctx.named('path')}`)
            return capitalize(`The ${fieldName} field must have ${ctx.named('length')} items`)
        }) as MessageFunctionCallable,
    },
